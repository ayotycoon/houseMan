...
export default function VacantRoomListing() {
	...
	const roomsQuery = useQuery({
		...
		queryFn: ({ queryKey }) => {
			...
			console.log('🔧 Rooms Query variables:', variables);
			return demoService.mockMyListings();
		},
	});
	const properties = [
		{value:"1", label:"395 rythmn lane"}
	]
	...
	return (
		<div className="flex flex-col gap-4 w-full">
			...
			<div className="flex flex-col gap-2">
				<Text variant="body2" className="text-muted-foreground text-sm">
					Active Property
				</Text>
				...
				<Card className="lg:col-span-2 flex flex-col p-6">
					...
					<div className={`grid gap-4 grid-cols-1 md:grid-cols-2`}>
						{roomsQuery?.data?.map((room) => (
							<RoomListingCard key={room.id} room={room} viewMode={"grid"} />
						))}
					</div>
